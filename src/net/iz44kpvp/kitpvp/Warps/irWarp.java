...
public class irWarp implements CommandExecutor
{
    public boolean onCommand(final CommandSender sender, final Command cmd, final String commandLabel, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("warp") && args.length == 0) {
            p.sendMessage(String.valueOf(String.valueOf(API.preffix)) + "§cSintaxe correta: /warp (challenge,fps,main,knock,mdr,rdm,,spawn)");
            return true;
        }
        if (args[0].equalsIgnoreCase("challenge")) {
            ...
            p.sendMessage(String.valueOf(String.valueOf(API.preffix)) + "§aVoce Foi Teleportado Para A Warp §cLava Challenge");
