...
public class Bruxa implements Listener
{
    ...
    public void onBruxa(final PlayerInteractEvent e) {
        ...
        if (Habilidade.getAbility(p).equalsIgnoreCase("Bruxa") && (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && p.getInventory().getItemInHand().getType() == Material.STICK) {
            if (Cooldown.add(p)) {
                API.MensagemCooldown(p);
                return;
            }
            Cooldown.add(p, 60);
            p.sendMessage(String.valueOf(String.valueOf(API.preffix)) + "§bVoc\u00ea deu alguns efeitos pros players mais proximos");
            for (final Entity pertos : p.getNearbyEntities(5.0, 5.0, 5.0)) {
                ...
                if (dar == 6) {
                    ((LivingEntity)pertos).addPotionEffect(new PotionEffect(PotionEffectType.SLOW_DIGGING, 160, 0), true);
                }
                if (dar == 7) {
                ...
                if (dar == 9) {
                    ((LivingEntity)pertos).addPotionEffect(new PotionEffect(PotionEffectType.SATURATION, 160, 0), true);
                }
